...
const TableCell = ({ value, classes = "" }) => (
  <div className={`px-4 py-2.5 rounded-xl border border-slate-100 bg-white min-h-[42px] flex items-center justify-center text-[13px] font-bold text-slate-700 transition-all hover:border-slate-200 shadow-sm ${classes}`}>
     {value || ""}
  </div>
);
...
const ProceduresTab = () => {
  ...
  const totalCharges = proceduresSection.reduce((acc, curr) => acc + curr.charge, 0);
  ...
  return (
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-3 text-[#009bf2] mb-2">
        ...
        <h2 className="text-[13px] font-extrabold uppercase tracking-tight">Procedures / Charges (CPT / HCPCS)</h2>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-[24px] border border-slate-100 shadow-sm overflow-hidden p-6 space-y-4">
           ...
           <div className="grid grid-cols-[1fr_1fr_1.5fr_1fr_1fr_1fr_1fr_1fr_1fr_40px] gap-3 px-2">
              {['DOS From', 'DOS To', 'CPT', 'Mod1', 'Mod2', 'Units', 'Charge', 'Dx Ptr', 'POS'].map(h => (
                 <span key={h} className="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center">{h}</span>
              ))}
              <span className="w-10" />
           </div>
           ...
           <div className="space-y-3">
              {proceduresSection.map((item, idx) => (
                 <div key={idx} className="grid grid-cols-[1fr_1fr_1.5fr_1fr_1fr_1fr_1fr_1fr_1fr_40px] gap-3 group items-center">
           ...
           <div className="pt-4 border-t border-slate-50 flex items-center justify-between px-2">
              <span className="text-[13px] font-black text-slate-700">Total Charge Amount</span>
              <div className="flex items-center gap-3">
                 <span className="text-[16px] font-black text-slate-800">${totalCharges.toFixed(2)}</span>
                 <span className="text-[11px] font-bold text-slate-300 uppercase tracking-widest">(Auto)</span>
              </div>
           </div>
        </div>
