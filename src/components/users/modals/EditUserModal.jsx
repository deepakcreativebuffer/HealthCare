  ...
  useEffect(() => {
    if (userData) {
      ...
      setFormData({
        ...
        documentType: userData.documentType || "ID Proof",
      });
    }
  }, [userData, isOpen]);
      ...
      <div className="bg-white w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-200 flex flex-col max-h-[95vh]">
        ...
        <div className="px-6 py-4 border-b border-slate-50 flex items-center justify-between bg-white sticky top-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-[16px] font-black text-slate-800 uppercase tracking-tight leading-none">Update User</h3>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar">
          ...
          <div className="bg-slate-50/50 p-4 rounded-xl border border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-0.5">Account Status</label>
              <div className="flex items-center gap-1.5 bg-white border border-slate-200 p-1 rounded-xl shadow-sm">
                {["Active", "Inactive"].map((status) => (
                  <button
                    ...
                    className={`flex-1 py-1.5 rounded-lg text-[10px] font-black transition-all uppercase tracking-widest ${formData.status === status
                      ? status === "Active"
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-0.5">Role</label>
              <div className="relative group">
                ...
                <select
                  className="w-full pl-9 pr-8 py-2.5 bg-white border border-slate-200 rounded-xl text-[12px] font-bold text-slate-700 outline-none focus:border-[#129FED]/30 transition-all appearance-none cursor-pointer shadow-sm"
                  value={formData.role}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <label className="relative flex flex-col items-center justify-center h-32 w-full border-2 border-dashed border-slate-100 rounded-xl bg-slate-50/30 hover:bg-slate-50 hover:border-blue-100 transition-all cursor-pointer group shadow-sm">
                  ...
                  {formData.photo ? (
                    ...
                    <>
                      <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-slate-300 shadow-sm border border-slate-100 group-hover:scale-110 transition-transform">
                        <ImageIcon size={18} />
                      </div>
                      <span className="text-[10px] font-medium text-slate-400 mt-2 uppercase">Replace Headshot</span>
                    </>
                  )}
                </label>
              </div>
                ...
                <label className="relative flex flex-col items-center justify-center h-32 w-full border-2 border-dashed border-slate-100 rounded-xl bg-slate-50/30 hover:bg-slate-50 hover:border-blue-100 transition-all cursor-pointer group shadow-sm">
                  ...
                  {formData.document ? (
                    ...
                    <>
                      ...
                      <span className="text-[10px] font-medium text-slate-400 mt-2 uppercase">Update Document</span>
                    </>
                  )}
                </label>
              </div>
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-slate-50 bg-slate-50/30 flex items-center gap-3 shrink-0">
          <button
            type="button"
            onClick={onClose}
