...
const StatCard = ({ label, value, subValue, icon: Icon, colorClass, borderClass, onClick }) => (
  <div onClick={onClick} className="flex-1 min-w-[140px] cursor-pointer hover:-translate-y-0.5 transition-transform">
    <Card className={`border-l-4 ${borderClass} py-2.5 px-3 h-full`}>
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-4 pb-10">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-[20px] font-extrabold text-gray-900 tracking-tight uppercase cursor-pointer hover:text-blue-600 transition-colors" onClick={() => openModal('EDIT_PROFILE')}>{m.name}</h1>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-6 gap-y-1 mt-1.5 text-[11px] font-medium text-gray-500">
              <span className="flex items-center gap-1.5 uppercase font-bold text-gray-400 cursor-pointer hover:text-gray-700 transition-colors" onClick={() => openModal('GENERIC_MESSAGE', { title: 'Patient ID', message: `Your Patient ID is ${m.id}. Have this ready when calling support.`})}><User className="w-3 h-3"/> PID: {m.id}</span>
              <span className="flex items-center gap-1.5 uppercase font-bold text-gray-400 cursor-pointer hover:text-gray-700 transition-colors" onClick={() => openModal('EDIT_PROFILE')}><Calendar className="w-3 h-3"/> DOB: {m.dob}</span>
              <span className="flex items-center gap-1.5 uppercase font-bold text-gray-400 cursor-pointer hover:text-gray-700 transition-colors" onClick={() => openModal('EDIT_PROFILE')}><Monitor className="w-3 h-3"/> GENDER: {m.gender}</span>
              <span className="flex items-center gap-1.5 font-bold text-[#129FED] cursor-pointer hover:underline" onClick={() => openModal('VIEW_LIST', { title: 'Insurance Info', items: [m.insurance] })}><Shield className="w-3 h-3"/> {m.insurance?.provider}</span>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <StatCard label="Next Appt" value={m.upcomingAppointment?.date} icon={Calendar} colorClass="bg-emerald-500" borderClass="border-l-emerald-500" onClick={() => openModal('VIEW_LIST', { title: 'Upcoming Appointments', items: [m.upcomingAppointment] })} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <Card className="p-3 space-y-3">
             ...
             <div className="space-y-2 max-h-[140px] overflow-x-hidden overflow-y-auto custom-scrollbar pr-1">
                {m.diagnoses?.map((d, i) => (
          ...
          <Card className="p-3 space-y-3">
             ...
             <div className="space-y-2.5 max-h-[140px] overflow-x-hidden overflow-y-auto custom-scrollbar pr-1">
          ...
          <Card className="p-3 space-y-3">
             ...
             <div className="space-y-2.5 mt-1 max-h-[140px] overflow-x-hidden overflow-y-auto custom-scrollbar pr-1">
                {m.documents?.map((d, i) => (
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          <Card className="p-0 overflow-hidden shadow-md border-t-4 border-t-blue-500">
            ...
            <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-8">
               <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-[12px]">
                    <div className="space-y-2">
                      <p className="flex justify-between cursor-pointer hover:text-blue-600" onClick={() => openModal('GENERIC_MESSAGE', { title: 'Visit Date', message: `Visit took place on context: ${m.lastVisit}` })}><span className="text-gray-400 flex items-center gap-1.5"><Calendar className="w-3 h-3"/> Date:</span> <span className="font-bold text-gray-900">{m.lastVisit}</span></p>
                      <p className="flex justify-between cursor-pointer hover:text-blue-600" onClick={() => openModal('VIEW_DETAILS', m.careTeam?.[0])}><span className="text-gray-400 flex items-center gap-1.5"><Stethoscope className="w-3 h-3"/> Provider:</span> <span className="font-bold text-[#129FED] underline">Dr. Emily Roberts</span></p>
                      <p className="flex justify-between cursor-pointer group" onClick={() => openModal('GENERIC_MESSAGE', { title: 'Chief Complaint', message: 'Patient reported ongoing chest pain, initiating further screening.' })}><span className="text-gray-400 flex items-center gap-1.5"><Activity className="w-3 h-3"/> Chief Complain:</span> <span className="font-bold text-gray-900 group-hover:text-[#129FED]">Chest Pain</span></p>
                      <p className="flex justify-between cursor-pointer group" onClick={() => openModal('GENERIC_MESSAGE', { title: 'Diagnosis Info', message: 'ICD-10 Code I20.9 - Angina Pectoris, unspecified' })}><span className="text-gray-400 flex items-center gap-1.5"><Activity className="w-3 h-3"/> Diagnosis:</span> <span className="font-bold text-gray-900 underline group-hover:text-[#129FED]">I20.9 - Angina Pectoris</span></p>
                    </div>
                  </div>
                  <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100/50 cursor-pointer hover:bg-blue-100/50 transition-colors" onClick={() => openModal('GENERIC_MESSAGE', { title: 'History of Present Illness', message: "51-year-old male with independent chest, fast pain for the past 3 days, described as pressure-like, radiating to left arm, associated with mild shortness of breath, ECG shows changes." })}>
                    <h4 className="text-[11px] font-extrabold text-[#129FED] uppercase mb-2 flex justify-between items-center">History of Present Illness <ExternalLink className="w-3 h-3" /></h4>
                    <p className="text-[12px] text-gray-600 leading-relaxed italic line-clamp-3">
               ...
               <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm text-center cursor-pointer hover:border-blue-300 transition-colors" onClick={() => openModal('VIEW_METRIC', { title: 'Blood Pressure', value: '120/80', subValue: 'mmHg', icon: Heart })}>
                      <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">BP</p>
                      <p className="text-[14px] font-black text-gray-900">120/80</p>
                    </div>
                    <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm text-center cursor-pointer hover:border-blue-300 transition-colors" onClick={() => openModal('VIEW_METRIC', { title: 'Heart Rate', value: '72', subValue: 'bpm', icon: Activity })}>
                      <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">HR</p>
                      <p className="text-[14px] font-black text-gray-900">72</p>
                    </div>
                    <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm text-center cursor-pointer hover:border-blue-300 transition-colors" onClick={() => openModal('VIEW_METRIC', { title: 'Temperature', value: '98.6', subValue: '°F', icon: Thermometer })}>
                      <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">Temp</p>
                      <p className="text-[14px] font-black text-gray-900">98.6°F</p>
                    </div>
                    <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm text-center cursor-pointer hover:border-blue-300 transition-colors" onClick={() => openModal('VIEW_METRIC', { title: 'SpO2', value: '98', subValue: '%', icon: Droplets })}>
                      <p className="text-[10px] text-gray-400 font-bold uppercase mb-1">O2</p>
                      <p className="text-[14px] font-black text-gray-900">98%</p>
                    </div>
                  </div>
                  
                  {/* ECG WAVEFORM VISUALIZATION */}
                  <div className="h-20 bg-gray-900 rounded-xl overflow-hidden relative flex items-center justify-center cursor-pointer hover:ring-2 hover:ring-emerald-400 transition-all" onClick={() => openModal('VIEW_ECG')}>
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" className="flex-1 h-8 text-[11px] font-bold" onClick={() => openModal('VIEW_ECG')}><Download className="w-3 h-3 mr-1.5"/> ECG REPORT</Button>
                    <Button variant="outline" className="flex-1 h-8 text-[11px] font-bold" onClick={() => openModal('VIEW_LIST', { title: 'Lab Reports & Documents', items: m.documents })}><FileText className="w-3 h-3 mr-1.5"/> DOCUMENTS</Button>
                  </div>
               </div>
            </div>
            
            <div className="px-5 py-4 bg-gray-50/30 border-t border-gray-100 flex items-center justify-between">
               ...
               <Button variant="ghost" className="h-7 text-[10px] font-black text-[#129FED] uppercase tracking-wider hover:bg-blue-50" onClick={() => openModal('GENERIC_MESSAGE', { title: 'Healthcare Goals', message: 'Active goals: Reduce blood pressure by 10 points over 6 months.' })}>More Details <ChevronRight className="w-3 h-3 ml-1"/></Button>
            </div>
          </Card>
          )}
          ...
          <Card className="p-0 overflow-hidden shadow-md border-t-4 border-t-blue-500">
            ...
            <div className="p-5 max-h-[400px] overflow-x-hidden overflow-y-auto custom-scrollbar">
               <div className="space-y-4">
                  {m.diagnoses?.map((d, i) => (
                     <div key={i} className="flex gap-4 p-4 border border-gray-100 rounded-xl hover:shadow-sm transition-shadow cursor-pointer bg-white" onClick={() => openModal('GENERIC_MESSAGE', { title: d.name, message: d.notes })}>
                        <div className="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center shrink-0">
                           <Activity className="w-5 h-5 text-blue-500" />
                        </div>
                        <div className="flex-1 min-w-0">
                           <div className="flex justify-between items-start mb-1 gap-2">
                              <h3 className="text-[13px] font-extrabold text-gray-900 truncate">{d.name}</h3>
                              <Badge className="bg-rose-50 text-rose-600 border-none text-[8px] uppercase shrink-0 px-1.5 py-0">{d.level}</Badge>
                           </div>
                           <p className="text-[11px] text-gray-400 font-bold mb-2 uppercase">Diagnosed on <span className="text-gray-700">{d.date}</span></p>
                           <p className="text-[12px] text-gray-600 italic bg-gray-50 p-2.5 rounded-lg border border-gray-100 line-clamp-2">{d.notes}</p>
                        </div>
                     </div>
                  ))}
               </div>
          ...
          <Card className="p-0 overflow-hidden shadow-md border-t-4 border-t-blue-500">
            ...
            <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[400px] overflow-x-hidden overflow-y-auto custom-scrollbar">
               {m.medicationsList?.map((med, i) => (
                  <div key={i} className="flex gap-3.5 p-4 border border-gray-100 rounded-xl hover:shadow-sm transition-shadow bg-white">
                     <div className="w-10 h-10 rounded-lg bg-blue-50 flex items-center justify-center shrink-0">
                        <Pill className="w-5 h-5 text-blue-500" />
                     </div>
                     <div className="min-w-0 flex-1">
                        <h3 className="text-[13px] font-extrabold text-gray-900 uppercase truncate">{med.name}</h3>
                        <p className="text-[11px] font-black text-[#129FED] mb-1 truncate">{med.dose} • {med.type}</p>
          ...
          <Card className="p-0 overflow-hidden shadow-md border-t-4 border-t-blue-500">
            ...
            <div className="p-0 max-h-[400px] overflow-x-hidden overflow-y-auto custom-scrollbar">
               <table className="w-full text-left text-[12px]">
                  <thead className="bg-gray-50/50 border-b border-gray-100 text-[10px] font-extrabold text-gray-400 uppercase tracking-widest sticky top-0 z-10 backdrop-blur-sm">
                     <tr>
                        <th className="py-3 px-5">Test Name</th>
                        <th className="py-3 px-4">Result</th>
                        <th className="py-3 px-4 hidden sm:table-cell">Ref Range</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                     {m.labResults?.map((l, i) => (
                        <tr key={i} className="hover:bg-gray-50/50 transition-colors cursor-pointer group" onClick={() => openModal('VIEW_METRIC', { title: l.name, value: l.value, subValue: '', icon: FlaskConical })}>
                           <td className="py-4 px-5 font-bold text-gray-900 group-hover:text-blue-600">{l.name}</td>
                           <td className="py-4 px-4 font-black text-gray-800">{l.value}</td>
                           <td className="py-4 px-4 font-medium text-gray-500 hidden sm:table-cell">{l.range}</td>
                           <td className="py-4 px-4 font-bold text-gray-400 hidden md:table-cell text-[10px] tracking-tight">{l.date}</td>
                           <td className="py-4 px-5 text-right">
                              <Badge className={`${l.status === 'Normal' ? 'bg-emerald-50 text-emerald-600' : l.status === 'High' ? 'bg-rose-50 text-rose-600' : 'bg-orange-50 text-orange-600'} border-none text-[9px] font-black uppercase px-2 py-0`}>{l.status}</Badge>
                           </td>
                        </tr>
                     ))}
                  </tbody>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
             <Card className="p-0 overflow-hidden shadow-sm">
                ...
                <div className="p-4 space-y-4 max-h-[180px] overflow-x-hidden overflow-y-auto custom-scrollbar pr-2">
          ...
          <Card className="p-0 overflow-hidden shadow-sm border-l-4 border-l-emerald-500 cursor-pointer hover:shadow-md transition-shadow" onClick={() => openModal('GENERIC_MESSAGE', { title: 'Discharge Active Status', message: `Discharge Status: ${m.dischargeSummary?.status}. The discharge instructions were released to patient.` })}>
             ...
             <div className="p-4 flex flex-col md:flex-row items-center justify-between gap-4">
                <p className="text-[12px] text-gray-600 font-medium italic truncate max-w-md"><span className="text-[10px] font-extrabold text-gray-400 not-italic uppercase mr-2">Primary Instruction:</span> "{m.dischargeSummary?.instruction}"</p>
                <Button className="bg-[#129FED] hover:bg-blue-600 text-white font-black text-[10px] h-8 shrink-0 py-0 uppercase tracking-widest px-4" onClick={(e) => { e.stopPropagation(); openModal('VIEW_DOCUMENT', { name: "Discharge_Summary.pdf" }); }}><Download className="w-3.5 h-3.5 mr-2"/> Download PDF</Button>
             </div>
          </Card>
        </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <Card className="p-3 space-y-3 cursor-pointer hover:shadow-md transition-shadow" onClick={() => openModal('VIEW_LIST', { title: 'Active Scripts', items: m.medicationsList })}>
             ...
             <div className="space-y-2.5 mt-1 max-h-[180px] overflow-x-hidden overflow-y-auto custom-scrollbar pr-1">
                {m.medicationsList?.map((med, i) => (
          ...
          <Card className="p-0 overflow-hidden shadow-sm">
             <div className="p-2.5 bg-gray-50 flex items-center justify-between border-b border-gray-100">
                ...
                <Badge className="bg-[#129FED] text-white border-none text-[8px] font-black px-2 py-0 cursor-pointer hover:bg-blue-600 transition-colors" onClick={() => openModal('GENERIC_MESSAGE', { title: 'Scheduling', message: 'You have 1 upcoming appointment.' })}>UPCOMING</Badge>
             </div>
             <div className="p-3 space-y-2 cursor-pointer hover:bg-gray-50 transition-colors rounded" onClick={() => openModal('GENERIC_MESSAGE', { title: 'Post-Op Follow Up', message: `Your follow up is firmly scheduled for ${m.upcomingAppointment?.date}.` })}>
                <div className="flex items-center gap-3">
                   <div className="w-10 h-10 rounded-xl bg-blue-50 flex flex-col items-center justify-center border border-blue-100 text-[#129FED]">
                      <span className="text-[10px] font-black uppercase">Dec</span>
                      <span className="text-[14px] font-black leading-none">10</span>
                   </div>
                   <div className="flex-1 min-w-0">
                      <p className="text-[13px] font-extrabold text-gray-900 truncate tracking-tight">{m.upcomingAppointment?.type}</p>
                      <p className="text-[10px] text-gray-400 font-bold truncate">Room 402 • FOLLOW-UP: 3 MONTHS</p>
                   </div>
                </div>
                <Button className="w-full h-8 bg-blue-50 text-[#129FED] hover:bg-blue-100 font-black text-[10px] border-none uppercase tracking-widest shadow-none" onClick={(e) => { e.stopPropagation(); openModal('VIEW_LIST', { title: 'Upcoming Visits', items: [m.upcomingAppointment] })}}>View Schedule</Button>
             </div>
          </Card>

          <div className="grid grid-cols-2 gap-3">
             <Card className="p-3 text-center space-y-2 cursor-pointer hover:border-blue-300 transition-colors group" onClick={() => openModal('MAP_VIEW', { name: "CVS Pharmacy #1234", address: '492 Pharmacy Ave, Suite B' })}>
             ...
             <Card className="p-3 text-center space-y-2 cursor-pointer hover:border-emerald-300 transition-colors group" onClick={() => openModal('VIEW_LIST', { title: 'Care Team Directory', items: m.careTeam })}>
